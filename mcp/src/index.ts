...
async function fetchLlmsTxt(): Promise<string> {
  ...
  if (!res.ok) throw new Error(`Failed to fetch llm.txt: ${res.status}`);
  return res.text();
}
    ...
    tools: [
      {
        ...
        description:
          "Search the Spirit Index for autonomous cultural agents. Filter by category, status, or minimum score. Returns ranked list of matching agents.",
        inputSchema: {
          type: "object",
          properties: {
            query: {
            ...
            category: {
              type: "string",
              description:
                "Filter by category (e.g., 'Autonomous Artist', 'Ecological DAO')",
            },
            status: {
              type: "string",
      ...
      },
      {
        ...
        description:
          "Get detailed dossier for a specific agent in the Spirit Index. Returns full scores, evidence, and analysis.",
        inputSchema: {
          type: "object",
          properties: {
            id: {
              type: "string",
              description:
                "Agent ID (slug form, e.g., 'botto', 'truth-terminal', 'plantoid')",
            },
          },
          required: ["id"],
        },
      ...
      },
      {
        ...
        description:
          "Get the top agents by a specific dimension or overall score.",
        inputSchema: {
          type: "object",
          properties: {
            dimension: {
      ...
      },
      {
        ...
        description:
          "Get information about the Spirit Index itself - what it is, how it works, and how to submit an agent.",
        inputSchema: {
          type: "object",
          properties: {},
        },
...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
  ...
  try {
    if (name === "spirit_index_search") {
      ...
      // Filter by min score
      if (args?.min_score) {
        agents = agents.filter((a) => a.total >= (args.min_score as number));
      }

      // Sort
      const sortBy = (args?.sort_by as string) || "total";
      agents.sort((a, b) => {
        if (sortBy === "total") return b.total - a.total;
        return (b.scores as any)[sortBy] - (a.scores as any)[sortBy];
      });
    ...
    if (name === "spirit_index_compare") {
      const id1 = args?.agent1 as string;
      const id2 = args?.agent2 as string;
      ...
      const comparison = dimensions
        .map((d) => {
          const s1 = (agent1.scores as any)[d];
          const s2 = (agent2.scores as any)[d];
...
| **TOTAL** | **${agent1.total}** | **${agent2.total}** | **${agent1.total > agent2.total ? agent1.name : agent2.total > agent1.total ? agent2.name : "Tie"}** |`,
    ...
    if (name === "spirit_index_leaderboard") {
      const dimension = (args?.dimension as string) || "total";
      ...
      if (dimension === "total") {
        ...
        agents.sort((a, b) => (b.scores as any)[dimension] - (a.scores as any)[dimension]);
      }
